...
> **测试数字**：当前实测 **397**（`dotnet test tests/Aurora.Tests.csproj`）；历史条目中的数字为该时点快照，不作当前值使用（口径规则见 CHANGELOG）。
...
| 维度 | 状态 |
| --- | --- |
| 单元测试 | **392 通过 / 0 失败 / 0 跳过**（2026-09-13 复验，`outputs/assoc-fix.trx`；23 个测试文件）。覆盖引擎竞态/跨淡/预载/RG、库差分与目录边界、ID3/GBK、LRC、安装清单与事务、安装安全、导入并发、更新解析、设置回退、关联启动路径、性能基线 |
...
| 8 | 你现有 `D:\Applications\Aurora`（v3.0.0，**无安装清单**）的迁移 | ✅ 已于 2026-09-13 清除完毕（含注册表、快捷方式、临时残留、用户数据；备份在 `outputs/uninstall-2026-09-13/`） |
| 9 | Assoc 关联路径修复的发布 | ✅ 已完成：**v3.0.2**（`main` = `develop` = `1ad9b7d`，修复关联命令指向 `.dll`） |
| 10 | **`Assoc.Register` 删除 `UserChoice` 的产品风险** | ✅ 已决策并实施（方案 A）：v3.0.3 移除该删除动作 + 接入"前往系统设置…"引导；审计报告见 `outputs/审计-关联UserChoice与安装审计目录-2026-09-13.md` |
| 11 | 审计目录 `.AuroraInstall-<hash>` 清理 | ✅ 已实施（v3.0.3）：提交后删载荷副本、弃用 `replaced-*` 冗余、开新事务回收旧事务目录、卸载彻底完成后删状态目录 |
| 12 | **v3.0.3 发布决策** | ⬜ 待拍板：改动已在 `develop`（未打 tag） |
...
- 安装事务在**安装目录同级**使用 `.AuroraInstall-<sha256>` 状态目录（事务日志 + 属主证明 + 独占锁）。
  v3.0.3 起的清理策略：提交成功后立即删除本次的载荷副本（`.old`/`.new`/`replaced-*`）只留日志；
  开新事务时回收被取代的旧事务目录；卸载彻底完成（安装目录已删）且无未决事务时删除状态目录本身。
  **仍会保留**的情形：卸载后安装目录里还留着用户文件（按设计不递归删除）、回滚事务的目录（留到下一次事务回收）、
  以及仅执行过 `/recover` 的空状态目录。
...
| ★新 | **安装事务重命名缺 NUL 终止符**（clean 全量复验新发现） | **已修复·真机已验证**（补终止符 + 扩展长度路径；此前深层目录会产生乱码文件名或报 206） |
| ★新 | **v3.0.0 安装包缺 `unins.dll`**：自带卸载器无法启动（`The application to execute does not exist: …\unins.dll`，退出码 154），Geek / "应用和功能"因共用 `UninstallString` 一并失败 | **v3.0.1 已修复**（打包清单补上该文件）。受影响的旧安装需手工清除：诊断与逐步方案见 `outputs/旧版卸载失败诊断与清除方案-2026-09-13.md`；本机已于 2026-09-13 清除完毕（31 项核对通过） |
| ★新 | **文件关联命令指向 `.dll`**（`Assembly.Location` 在框架依赖应用里返回 DLL 路径）→ "打开方式 → Aurora" 无法启动 | **已修复并发布（v3.0.2）**：改用 `Environment.ProcessPath`，补 3 例回归 |
| ★新 | **`Assoc.Register` 静默删除 `UserChoice`**（首启即抢走 9 种格式的默认关联，卸载不还原；且部分系统该键受保护、删除会失败） | **已修复（v3.0.3，方案 A）**：移除删除动作，只写自己的 ProgID/扩展名默认值；设置里新增"前往系统设置…"接入 `OpenDefaultAppsSettings()`。`Assoc.IsAssociated()` 仍未接入 UI |
| ★新 | **审计目录无清理路径**（每个事务留一个子目录、旧文件存两份 `.old` + `replaced-*`；实测一轮验证累积 ≈276 MB） | **已修复（v3.0.3）**：见 §9.1 的清理策略；`tools/machine-verify/verify_assoc_stage4.py` 与 5 例单元测试覆盖 |
| ★新 | **`Assoc.Unregister` 不清理 `OpenWithProgids` 中的自身条目**（卸载器会清理，`/unassociate` 不会） | ⬜ 未修（真机脚本已能检出，属已知不一致） |
