...
> **测试数字**：当前实测 **392**（`dotnet test tests/Aurora.Tests.csproj`）；历史条目中的数字为该时点快照，不作当前值使用（口径规则见 CHANGELOG）。
...
| 9 | **Assoc 修复的发布决策** | ⬜ 待拍板：修复已在 `develop`（未发布），是否打 **v3.0.2**（该缺陷影响已发布的 v3.0.1：关联命令指向 `.dll`） |
| 10 | **`Assoc.Register` 会删除 9 个扩展名的 `FileExts\<ext>\UserChoice`** | ⬜ 待拍板：这会**重置用户已选的默认播放器**（属产品行为风险，本次按"只修 dll 缺陷"的范围未动） |
...
| ★新 | **文件关联命令指向 `.dll`**（`Assembly.Location` 在框架依赖应用里返回 DLL 路径）→ "打开方式 → Aurora" 无法启动 | **已修复（未发布）**：改用 `Environment.ProcessPath`，补 3 例回归；测试 389 → 392 |
