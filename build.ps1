...
#  顺序：主程序(WPF) -> 卸载器(WinForms) -> 安装器(WinForms, SingleFile)
# ============================================================
...
Write-Host "=== [1/3] AuroraPlayer (WPF, .NET 10) ===" -ForegroundColor Cyan
dotnet build AuroraPlayer.csproj -c Release -v minimal --nologo
if ($LASTEXITCODE -ne 0) { Write-Host "BUILD FAILED" -ForegroundColor Red; exit 1 }

Write-Host "`n=== [2/3] unins (WinForms, .NET 10) ===" -ForegroundColor Cyan
dotnet build unins.csproj -c Release -v minimal --nologo
if ($LASTEXITCODE -ne 0) { Write-Host "BUILD FAILED" -ForegroundColor Red; exit 1 }

Write-Host "`n=== [3/3] AuroraPlayer-Setup (WinForms, .NET 10, SingleFile) ===" -ForegroundColor Cyan
