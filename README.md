...
![Aurora 主界面 · 浅色主题（黑胶播放页）](assets/screenshot-main.png)

<sub>深色主题与播放中（黑胶旋转 + 歌词）效果更佳，欢迎下载体验 🎵</sub>
...
| 产物 | 适用 | 说明 |
| --- | --- | --- |
| **AuroraPlayer-Setup.exe** | 推荐大多数用户 | 单文件安装包（约 30 MB，内嵌 WinRT 投影组件）。可选安装目录、格式关联、自带卸载器。**需 .NET 10 Desktop Runtime** |
...
命令行手动关联/取消关联（无需管理员，写 HKCU）：

```
AuroraPlayer.exe /associate     # 关联全部 9 种格式
AuroraPlayer.exe /unassociate   # 取消关联并清理注册表项
```
...
| 联网匹配 | 数据源级启停（酷狗/网易云按序降级）、按格式开关、缓存统计与一键清除（`%LOCALAPPDATA%\Aurora\Cache\`） |
| 音频诊断 | 设置 → 音频诊断：解码格式/采样率/位深/声道/ReplayGain/输出设备快照，一键复制反馈、打开日志文件夹 |
